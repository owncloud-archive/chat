Chat.och.api = {
    command : {
        greet : function(success){
            Chat.och.api.util.doRequest({"type" : "command::greet::request", "data" : { "user" : OC.currentUser, "session_id" : Chat.och.sessionId, "timestamp" : (new Date).getTime() / 1000 }}, success);
        },
        join : function(convId, success){
            Chat.och.api.util.doRequest({"type" : "command::join::request", "data" : { "conv_id": convId, "timestamp" : (new Date).getTime() / 1000, "user" : OC.currentUser, "session_id" : Chat.och.sessionId }}, success);
        },
        invite : function(userToInvite, convId, success, error){
            Chat.och.api.util.doRequest({"type" : "command::invite::request", "data" : { "conv_id" : convId, "timestamp" : (new Date).getTime() / 1000, "user_to_invite" : userToInvite , "user" : OC.currentUser, "session_id" : Chat.och.sessionId }},success,error);
        },
        sendChatMsg : function(msg, convId, success){
            Chat.och.api.util.doRequest({"type" : "command::send_chat_msg::request", "data" : {"conv_id" : convId, "chat_msg" : msg, "user" : OC.currentUser, "session_id" : Chat.och.sessionId, "timestamp" : (new Date).getTime() / 1000 }}, success);
        },
        leave : function(convId, success){
            Chat.och.api.util.doRequest({"type" : "command::leave::request", "data" : { "conv_id" : convId, "user" : OC.currentUser, "session_id" : Chat.och.sessionId, "timestamp" : (new Date).getTime() / 1000 }}, success);
        },
        online : function(success){
            Chat.och.api.util.doRequest({"type" : "command::online::request", "data" : { "user" : OC.currentUser, "session_id" : Chat.och.sessionId, "timestamp" : (new Date).getTime() / 1000}}, function(){});
        }
    },
    on : {
        invite : function(data){
            ...
            Chat.app.view.addConv(data.conv_id, [data.user, OC.currentUser], backend);
    ...
    deleteConversation(conversationID);
        ...
        getPushMessages : function(success){
            $.post(OC.Router.generate('chat_och_api'), {"JSON" : JSON.stringify({"type" : "push::get::request", "data" : { "user" : OC.currentUser, "session_id" : Chat.och.sessionId}})}, function(data){
                success(data);
            });
        },
        deletePushMessages : function(ids, success){
            $.post(OC.Router.generate('chat_och_api'), {"JSON" : JSON.stringify({"type" : "push::delete::request", "data" : {"user" : OC.currentUser, "session_id" : Chat.och.sessionId, ids: ids}})}, function(data){
               success();
            });
        },
    }
